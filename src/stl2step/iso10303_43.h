 ...
 *
 * <P><B>1. Область применения</B></P>
 * <P>В настоящем стандарте специфицированы конструкции объединенных обобщенных ресурсов для
 * структур представления. Это - собрания элементов данных об изделии, которые группируются вместе
 * для описания свойств и аспектов изделий.<BR>
 ...
 *
 * <P><B>4. Схема Representation (представление)</B></P>
...
//  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(compound_item_definition, list_representation_item);
//  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(compound_item_definition, set_representation_item);
};
...
public:
  ...
  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(founded_item_select, founded_item);
  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(founded_item_select, representation_item);
};
...
public:
  ...
  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(representation_or_representation_reference, representation);
  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(representation_or_representation_reference, representation_reference);
};
...
//STEP_TYPE_SELECT_CONSTRUCTOR_DEF(transformation, functionally_defined_transformation);
  STEP_TYPE_SELECT_CONSTRUCTOR_DEF(transformation, item_defined_transformation);
};
 ...
 */
class founded_item : public STEP_ENTITY {
...
public:
  ...
  // Дополнить массив
  void add_uncert(const uncertainty_measure_with_unit* x);
};
 ...
 *
 */
class item_defined_transformation : public STEP_ENTITY {
 ...
 * \verbatim
 ...
 \endverbatim
 *
 * Объект <B>\ref representation_item</B> представляет элемент представления. На объект <B>\ref representation_item</B> ссылается
 * один или более экземпляров объекта <B>\ref representation</B>, или же объект <B>\ref representation_item</B> используется в определении
 * другого объекта <B>\ref representation_item</B>.
 */
class representation_item : public STEP_ENTITY {
 ...
 *
 */
class representation_reference : public STEP_ENTITY {
private:
 ...
 * \verbatim
 ...
 \endverbatim
 *
 * Объект   uncertainty_measure_with_unit  является  таким  подтипом  объекта,
 * посредством которого задается погрешность, применяемая к мере данного типа.
 * Погрешность,    представляемая    объектом   uncertainty_measure_with_unit,
 * применяется  к  каждому объекту representation_item, в котором используется
 * тип меры, заданный объектом, играющим роль атрибута value_component объекта
 * uncertainty_measure_with_unit, в следующих случаях:<BR>
 * * представляющий элемент представления объект representation_item входит во
 * множество,  играющее  роль  атрибута  items  объекта  uncertainty_assigned_
 * representation;<BR>
 * * элемент   представления,   представляемый   объектом  representation_item,
 * основан  в контексте, представленном объектом global_unit_assigned_context<BR>
 * * экземпляр  объекта  representation_item  является  экземпляром  объекта
 * qualified_representationjtem.<BR>
 * 
 */
...
public:
  ...
  uncertainty_measure_with_unit(const measure_value* Value, const unit* Unit, const std::string& Name, const std::string& Description = "")
    : measure_with_unit(Value, Unit), name(Name), description(Description) {
...
// 4.5.5 Функция using_representations
